...
-- Local helper to route optional debug prints into the addon's Debug system
local function HBDP_LogDebug(msg)
    if BeastAndBow and BeastAndBow.Debug and BeastAndBow.Debug.Debug then
        BeastAndBow.Debug.Debug(msg, "HBD-PINS")
    end
    -- otherwise: intentionally silent (these were noisy debug prints)
end
...
function worldmapProvider:HandlePin(icon, data)
    HBDP_LogDebug(string.format("HandlePin | icon: %s, openMapID: %s, pinMapID: %s", tostring(icon:GetName()), tostring(self:GetMap():GetMapID()), tostring(data.uiMapID)))
...
function pins:AddWorldMapIconWorld(ref, icon, instanceID, x, y, showFlag, frameLevel)
    HBDP_LogDebug(string.format("AddWorldMapIconWorld | ref: %s, icon: %s, instanceID: %s, x: %.2f, y: %.2f", tostring(ref), tostring(icon:GetName()), tostring(instanceID), x, y))
    if not ref then
...
function pins:AddWorldMapIconMap(ref, icon, uiMapID, x, y, showFlag, frameLevel)
    HBDP_LogDebug(string.format("AddWorldMapIconMap | ref: %s, icon: %s, uiMapID: %s, x: %.2f, y: %.2f", tostring(ref), tostring(icon:GetName()), tostring(uiMapID), x, y))
    if not ref then
...
function pins:RemoveWorldMapIcon(ref, icon)
    HBDP_LogDebug(string.format("RemoveWorldMapIcon | icon: %s", tostring(icon:GetName())))
